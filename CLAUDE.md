...
4. **Minimum-sample hardgate** for all reliability aggregates (modal pay day, delay rate). Below threshold show locked-preview empty state with CTA. (ADR-0004)
...
src/app/api/...          submit, search, roles/suggest, admin/*, report, companies
...
scripts/                 Operational scripts (classify, logos, seed)
...
- **Postgres pool size is load-bearing** in `src/lib/db.ts`. `CHUNK` in `src/app/admin/queue/Queue.tsx` must match. Any change requires reading `docs/database.md` and ADR-0002 first.
- **Database `statement_timeout`** is set on the postgres role via `alter database`. The postgres.js startup param does NOT propagate through Supavisor - the DB default is the real ceiling.
- **A `pg_cron` cleanup job** terminates Supavisor backends stuck in `active+ClientRead` over a threshold. Don't disable without replacing.
...
- For production probes use **Playwright MCP** (`mcp__plugin_playwright_playwright__browser_navigate` + `browser_snapshot` / `browser_network_requests`).
- **`GEMINI_API_KEY` is in `.env.local`.** For any bulk classification task (spam audits, role/company normalization, fuzzy de-dup, freetext bucketing, content moderation triage) prefer LLM-as-judge over hand-written regex/heuristics - regex under-flags significantly on this dataset. Reference pattern: `gemini-2.5-pro` with `responseMimeType: application/json` for structured ratings, then dump CSV (not JSON) for human review.
...
- **Supabase** (`mcp__supabase__*`) - project: **`betonkemon`**, ref `<SUPABASE_PROJECT_REF>`, region `ap-northeast-1`. Pass `project_id: "<SUPABASE_PROJECT_REF>"` on every call. Same org has other projects - never touch those. Default for: live entry counts, schema inspection (`list_tables`), advisors / log triage (`get_advisors`, `get_logs`), migrations (`apply_migration`), one-off SQL (`execute_sql`). Destructive mutations (`DELETE`/`UPDATE` on `salary_entries`/`roles`/`companies`) go through the `database-entry-cleanup` skill, never raw `execute_sql`.
